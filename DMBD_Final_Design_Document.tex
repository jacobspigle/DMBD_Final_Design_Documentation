...
\addcontentsline{toc}{chapter}{Technical Goals}
	...
	\begin{itemize}
		\item Interactive virtual board that acts as the role-playing game's Encounter Arena 
		\item Encounter Creator
		\begin{itemize}
			\item Variables for players, enemies, and environmental structures that are able to be placed on the board
			\item Ability to alter enemy statistics and behavior	
		\end{itemize}
		\item Battle-mode 
		\begin{itemize}
			\item Players and DM's can play through a created encounter that follows the standard d20 rules.
			\item Turn-based system that allows for as many real d20 moves as reasonable
			\item Enemy AI
		\end{itemize}
		\item Encounter Browser
		\item User Arenas (User Profiles)
		\item Encounter Stress Tester
	\end{itemize}
