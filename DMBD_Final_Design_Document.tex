...
\addcontentsline{toc}{chapter}{Research}
	...
	\section{Game A.I.}
		...
		The drawbacks of finite state machines are that they are limited by their simplicity. The machine can only be in one state at once, which means you have to pick the states in you FSM to be states that can never occur simultaneously in your A.I. Additionally, with an FSM, you cannot look forward to future actions to make a decision. You cannot backtrack or plan ahead. 
		...
		\begin{figure}[H]
			...
			\caption{Behavior tree for a search and grasp robot}
			\label{fig: BT Robot}
		\end{figure}
		...
		The drawback to the approach is that it is quite difficult to adapt to the D20 system, as a unit's utility is very hard to gauge accurately at a given moment. For this reason, a utility driven A.I. was not truly considered for use in Critical Encounters. 
